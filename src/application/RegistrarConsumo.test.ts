...
describe('RegistrarConsumo', () => {
  ...
  it('grava o alimento que o aluno escolheu entre as alternativas', async () => {
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
  ...
  it('acumula os itens da mesma refeição num registro só', async () => {
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i2', alimento: 'Frango' })
  ...
  it('separa refeições diferentes', async () => {
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
    await registrar.executar(HOJE, { refeicao: 2, itemDeRefeicaoId: 'r2i1', alimento: 'Iogurte' })

    expect((await refeicoes()).map((r) => r.numero)).toEqual([1, 2])
  })

  it('trocar de alternativa substitui, em vez de registrar duas', async () => {
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Batata' })
  ...
  it('desmarcar tira o item — quem tocou por engano precisa poder desfazer', async () => {
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i2', alimento: 'Frango' })
    await registrar.executar(HOJE, { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: null })
  ...
  it('recusa data inválida em vez de corromper o índice do vault', async () => {
    await expect(
      registrar.executar('24/08/2026', { refeicao: 1, itemDeRefeicaoId: 'r1i1', alimento: 'Arroz' })
    ).rejects.toThrow()
  })
})
